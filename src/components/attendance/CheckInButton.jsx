...
const CheckInButton = ({todayRecord, onAction }) => {
  ...
  return (
    <div className='absolute bottom-4  right-4  flex flex-col  z-1'>
      <button onClick={handleAttendance} disabled={loading} className={`w-full max-w-xs flex justify-between items-center gap-8 p-4  rounded-xl bg-linear-to-br text-white ${isCheckedIn ? "from-slate-200 to-slate-900" : "from-indigo-600 to-indigo-700"}`}>
          ...
          <div className='relative flex flex-col items-center text-center'>
            <h2 className='text-lg font-bold mb-1'>{loading? "Processing" : isCheckedIn ? "Clock Out" : "Clock In"}</h2>
            <p className='text-xs opacity-80'>{isCheckedIn ? "Click to enter your shift" : "Start your work day"}</p>
          </div>
      </button>
    </div>
  )
